...
func (a *ChunkAssembler) Push(msg ImmutableMessage) (ImmutableMessage, bool, error) {
	idx, total, chunked, err := parseChunkMetadata(msg)
	if !chunked {
...
func parseChunkMetadata(msg BasicMessage) (idx int, total int, chunked bool, err error) {
	...
	if !hasIndex || !hasTotal {
		return 0, 0, true, errors.Wrapf(ErrCorruptedChunk,
