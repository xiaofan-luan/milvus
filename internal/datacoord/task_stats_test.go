...
func (s *statsTaskSuite) TestSetJobInfoSortResultManifestHandling() {
	...
	testCases := []struct {
		...
		{
			// A birth commit has no current manifest to compare against, so the
			// successor fence is skipped — but an unparseable manifest must still
			// never reach SegmentInfo.
			name:           "birth_unparseable_result_rejected",
			current:        "",
			base:           "",
			result:         "not-a-manifest",
			expectManifest: "",
			expectErr:      errStatsResultStale,
		},
		{
			// The same guard applies to a materialized segment whose result is
			// garbage: reject before the successor comparison.
			name:           "unparseable_result_rejected",
			current:        currentManifest,
			base:           currentManifest,
			result:         "not-a-manifest",
			expectManifest: currentManifest,
		...
		{
			// UnmarshalManifestPath is a bare json.Unmarshal, so "{}" decodes fine
			// but yields an empty base path — structurally valid, semantically
			// meaningless. It must not reach SegmentInfo on a birth commit.
			name:           "birth_empty_base_path_rejected",
			current:        "",
			base:           "",
			result:         `{}`,
			expectManifest: "",
		...
		{
			// "null" also decodes without error, leaving zero values.
			name:           "birth_null_manifest_rejected",
			current:        "",
			base:           "",
			result:         `null`,
			expectManifest: "",
		...
		{
			// A negative version is nonsense; version 0 (packed.ManifestEarliest)
			// is legal and is covered by true_birth_adopted.
			name:           "birth_negative_version_rejected",
			current:        "",
			base:           "",
			result:         `{"base_path":"files/insert_log/1/2/1179","ver":-1}`,
			expectManifest: "",
			expectErr:      errStatsResultStale,
		},
		{
