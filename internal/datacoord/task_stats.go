...
func updateStatsResultIfManifestMatches(ctx context.Context, segmentID, taskID int64, result *workerpb.StatsResult) UpdateOperator {
	return func(modPack *updateSegmentPack) bool {
		...
		// Parse fence: a non-empty result manifest must be a well-formed manifest
		// path before it can reach SegmentInfo. This is the only guard a BIRTH
		// commit gets — with no current manifest there is nothing to compare
		// against, so the successor fence below is skipped and a malformed value
		// (e.g. "not-a-manifest") would otherwise be written verbatim.
		//
		// SegmentInfo.ManifestPath only ever holds a loon manifest: it is created
		// solely by packed.MarshalManifestPath behind a StorageV3 guard (see
		// segment_manager.go), and every other write either round-trips that value
		// or comes from a packed API. A non-empty value that fails to parse is
		// therefore genuinely corrupt, not a foreign storage format.
		//
		// NOTE: this deliberately does NOT verify that the manifest belongs to this
		// segment. DataCoord cannot derive the sort target's expected base path
		// here, and guessing the layout risks rejecting every legitimate sort birth.
		if resultManifest != "" {
			resultBase, resultVer, err := packed.UnmarshalManifestPath(resultManifest)
			if err != nil {
				return reject(merr.Wrapf(err, "stats result manifest %q is not a parseable manifest path", resultManifest))
			}
			// UnmarshalManifestPath is a bare json.Unmarshal: "{}", "null" and a
			// negative version all decode without error. Check the decoded fields
			// too, otherwise a birth commit writes a structurally valid but
			// meaningless pointer into SegmentInfo. Version 0 IS legal — it is
			// packed.ManifestEarliest, what a freshly allocated segment starts at.
			if resultBase == "" || resultVer < 0 {
				return reject(merr.WrapErrServiceInternalMsg(
					"stats result manifest %q decodes to an empty base path or negative version (base=%q, ver=%d)",
					resultManifest, resultBase, resultVer))
			}
		}
