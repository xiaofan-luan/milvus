...
func (ss *SuffixSnapshot) isTSKey(key string) (typeutil.Timestamp, bool) {
	...
	ts, err := strconv.ParseUint(key[idx+1:], 10, 64)
	if err != nil {
		return 0, false
	}
	return ts, true
...
func (ss *SuffixSnapshot) removeExpiredKvs(now time.Time) error {
	...
	cleanFn := func(curOriginalKey string) error {
		// Check if the original key has been deleted
		originalValue, err := ss.MetaKv.Load(curOriginalKey)
		if err != nil {
			return err
		}
		if ss.isTombstone(originalValue) {
