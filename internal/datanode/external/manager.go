...
func isKnownRetryableRefreshError(err error) bool {
	...
	// the segcore code table flags retriable, and ErrStorageTransient — which is
	// what a loon FFI failure (transaction conflict, manifest commit contention)
	// now carries end-to-end, including through packed's storage relabels. A
	// permanent segcore code (FieldIDInvalid "column not found") stays
	// non-retriable and falls through to Failed.
