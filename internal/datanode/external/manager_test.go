...
func TestExternalCollectionManager_ClassifiesExecutionFailures(t *testing.T) {
	...
	// A loon FFI failure, shaped exactly as packed produces it: HandleLoonFFIResult
	// labels the transient loon failure ErrStorageTransient, and the manifest-commit
	// call site relabels it with merr.WrapErrStorage to attach a storage wire code
	// (manifest_commit.go / transaction.go do exactly this). That relabel must not
	// downgrade the cause — otherwise a recoverable transaction conflict fails the
	// whole refresh job instead of being re-dispatched.
	wrappedLoonID := int64(5003)
	err = manager.SubmitTask(clusterID,
		&datapb.RefreshExternalCollectionTaskRequest{TaskID: wrappedLoonID, TaskVersion: 1},
		func(taskCtx context.Context) (*datapb.RefreshExternalCollectionTaskResponse, error) {
			loonErr := merr.WrapErrStorageTransient(packed.ErrLoonTransient,
				"FFI operation failed: transaction conflict")
			return nil, merr.WrapErrStorage(loonErr, "commit manifest begin")
		})
	require.NoError(t, err)
	require.Eventually(t, func() bool {
		info := manager.Get(clusterID, wrappedLoonID)
		return info != nil && info.State == indexpb.JobState_JobStateRetry
	}, time.Second, 10*time.Millisecond)
}
