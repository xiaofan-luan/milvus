...
IndexFactory::ScalarIndexLoadResource(
    ...
    if (index_type == milvus::index::ASCENDING_SORT) {
        ...
        // load DOES rebuild on the heap is the rank directories (the quad
        // wavelet's per-word relative counts are ~the size of the wavelet
        // words themselves), plus the wrapper's per-row arrays (row_len_ 4 B +
        // null bitmap 1 bit). That resident total is strictly LESS than the
        // blob (the directories mirror only the wavelet section), so
        // index_size is a simple, true upper bound — no format-dependent
        ...
        if (mmap_enable) {
            ...
            // Deserialize keeps the whole blob owned in RAM (the views point
            // into it) plus the same rebuilt directories (< 1x blob):
            // final <= 2x. Peak adds the transient read buffer while the
            // owned copy is made: <= 3x.
            request.final_memory_cost = 2 * index_size_in_bytes;
            request.final_disk_cost = 0;
            request.max_memory_cost = 3 * index_size_in_bytes;
            request.max_disk_cost = 0;
        }
        request.has_raw_data = false;
