...
class SetElement : public MultiElement {
    static_assert(!std::is_same_v<T, bool>,
                  "Use SetElement<bool> specialization to avoid "
                  "ankerl::unordered_dense::set<bool> ASan issues");
...
class SimdBatchElement : public MultiElement {
 ...
 public:
    // Caller must pass sorted values. Go rewriter (sortTermValues) handles this
    // for production; unit tests must pre-sort before constructing.
    ...
    // Per-row fallback: binary search (vals_ must be sorted by caller)
    bool
    In(const ValueType& value) const override {
        T v = std::get<T>(value);
...
GetElementValues(const std::shared_ptr<MultiElement>& ptr) {
    ...
    if (auto p = std::dynamic_pointer_cast<FlatVectorElement<T>>(ptr)) {
        return p->values_;
    }
