...
PhyTermFilterExpr::ExecVisitorImplForData(EvalCtx& context) {
    ...
    if (!arg_inited_) {
        ...
        if constexpr (std::is_same_v<T, std::string> ||
            ...
            arg_set_ = std::make_shared<FlatVectorElement<T>>(vals);
        } else {
        ...
        // For string_view, underlying elements are stored as std::string;
        // cache as vector<string> to own the data and avoid dangling views.
        if constexpr (std::is_same_v<T, std::string_view>) {
            cached_skip_elements_ = GetElementValues<std::string>(arg_set_);
        } else {
    ...
    auto skip_index_func = [&cached_elements = cached_skip_elements_](
                               ...
                               FieldId field_id,
                               int64_t chunk_id) {
        // For string_view, cached_skip_elements_ stores vector<string>
        // to own the data. Use CanSkipInQuery<string> accordingly.
        if constexpr (std::is_same_v<T, std::string_view>) {
            auto* elements =
                std::any_cast<std::vector<std::string>>(&cached_elements);
            if (elements == nullptr) {
                return false;
            }
            return skip_index.CanSkipInQuery<std::string>(
                field_id, chunk_id, *elements);
        } else {
