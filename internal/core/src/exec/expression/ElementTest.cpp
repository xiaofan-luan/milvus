...
TEST(SimdBatchElementTest, GetElements) {
    std::vector<int32_t> vals = {3, 1, 2};
    SimdBatchElement<int32_t> sb(vals);
    auto result = sb.GetElements();
    // vals_ is sorted internally after dedup, so compare sorted
...
// Note: SetElement<bool> specialization was removed. Bool IN expressions
// are rewritten to equality/always-true at the proxy layer (Go rewriter).
// The generic SetElement<bool> (ankerl hash set) is tested as fallback.
...
TEST(SimdBatchElementTest, WrongTypeVariant) {
    ...
    // int32_t(1) matches, but int64_t(1), float(1.0f) etc. should not
    EXPECT_TRUE(sb.In(VT(int32_t(1))));
    EXPECT_FALSE(sb.In(VT(int64_t(1))));
    EXPECT_FALSE(sb.In(VT(int8_t(1))));
    EXPECT_FALSE(sb.In(VT(float(1.0f))));
    EXPECT_FALSE(sb.In(VT(double(1.0))));
    EXPECT_FALSE(sb.In(VT(std::string("1"))));
    EXPECT_FALSE(sb.In(VT(bool(true))));
    EXPECT_FALSE(sb.In(VT(std::monostate{})));
}

TEST(SimdBatchElementTest, WrongTypeVariantFloat) {
    SimdBatchElement<float> sb(std::vector<float>{1.0f});
    EXPECT_TRUE(sb.In(VT(1.0f)));
    EXPECT_FALSE(sb.In(VT(1.0)));  // double
    EXPECT_FALSE(sb.In(VT(int32_t(1))));
}
