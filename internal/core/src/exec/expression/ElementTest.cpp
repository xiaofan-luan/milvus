...
TEST(SimdBatchElementTest, Int8) {
    ...
    EXPECT_FALSE(sb.In(VT(int8_t(2))));
    EXPECT_FALSE(sb.In(VT(int8_t(100))));
}
...
TEST(SimdBatchElementTest, Int16) {
    ...
    EXPECT_FALSE(sb.In(VT(int16_t(2))));
}
...
TEST(SimdBatchElementTest, Int64) {
    std::vector<int64_t> vals = {1, -1, 0, 1000000000LL, -1000000000LL};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<int64_t> sb(vals);
    for (auto v : vals) {
        EXPECT_TRUE(sb.In(VT(v)));
    }
    EXPECT_FALSE(sb.In(VT(int64_t(2))));
}

TEST(SimdBatchElementTest, Float) {
    std::vector<float> vals = {1.5f, -1.5f, 0.0f, 1e10f};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<float> sb(vals);
    for (auto v : vals) {
        EXPECT_TRUE(sb.In(VT(v)));
    }
    EXPECT_FALSE(sb.In(VT(1.6f)));
    EXPECT_FALSE(sb.In(VT(0.1f)));
}

TEST(SimdBatchElementTest, Double) {
    std::vector<double> vals = {1.5, -1.5, 0.0, 1e100};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<double> sb(vals);
    for (auto v : vals) {
        EXPECT_TRUE(sb.In(VT(v)));
    }
    EXPECT_FALSE(sb.In(VT(1.6)));
}

TEST(SimdBatchElementTest, Empty) {
    SimdBatchElement<int32_t> sb(std::vector<int32_t>{});
    EXPECT_TRUE(sb.Empty());
    EXPECT_FALSE(sb.In(VT(int32_t(0))));
}

TEST(SimdBatchElementTest, SingleElement) {
    SimdBatchElement<int64_t> sb(std::vector<int64_t>{42});
    EXPECT_TRUE(sb.In(VT(int64_t(42))));
    EXPECT_FALSE(sb.In(VT(int64_t(0))));
    EXPECT_FALSE(sb.In(VT(int64_t(43))));
}
...
VerifyFilterChunk(const std::vector<T>& in_vals,
    ...
    in_sorted.erase(std::unique(in_sorted.begin(), in_sorted.end()),
                    in_sorted.end());
...
TEST(SetElementStringTest, Basic) {
    ...
    EXPECT_EQ(fh.Size(), 3);
    EXPECT_TRUE(fh.In(VT(std::string("hello"))));
    EXPECT_TRUE(fh.In(VT(std::string("world"))));
    EXPECT_FALSE(fh.In(VT(std::string("Hello"))));
    EXPECT_FALSE(fh.In(VT(std::string(""))));
}

TEST(SetElementStringTest, StringView) {
    SetElement<std::string> fh(std::vector<std::string>{"alpha", "beta"});
    EXPECT_TRUE(fh.In(VT(std::string_view("alpha"))));
    EXPECT_FALSE(fh.In(VT(std::string_view("gamma"))));
}
...
TEST(SetElementStringTest, EmptyString) {
    SetElement<std::string> fh(std::vector<std::string>{""});
    EXPECT_TRUE(fh.In(VT(std::string(""))));
    EXPECT_FALSE(fh.In(VT(std::string("x"))));
}
...
TEST(SetElementStringTest, SharedPrefixCollision) {
    ...
    SetElement<std::string> fh(vals);
    for (auto& v : vals) {
        EXPECT_TRUE(fh.In(VT(v))) << "should find: " << v;
    }
    EXPECT_FALSE(fh.In(VT(prefix + "999")));
    EXPECT_FALSE(fh.In(VT(prefix)));
}
...
TEST(SetElementStringTest, Duplicates) {
    ...
    EXPECT_EQ(fh.Size(), 2);
    EXPECT_TRUE(fh.In(VT(std::string("dup"))));
    EXPECT_TRUE(fh.In(VT(std::string("other"))));
}
...
TEST(GetElementValuesTest, FlatVectorElementReturnsValues) {
    auto elem = std::make_shared<FlatVectorElement<int32_t>>(
        std::vector<int32_t>{1, 2, 3});
    auto result =
        GetElementValues<int32_t>(std::static_pointer_cast<MultiElement>(elem));
    EXPECT_EQ(result.size(), 3);
    EXPECT_EQ(result, (std::vector<int32_t>{1, 2, 3}));
}
...
TEST(CrossValidationTest, Int8SimdVsSet) {
    std::vector<int8_t> vals = {-5, 0, 5, 10, -128, 127};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<int8_t> sb(vals);
    SetElement<int8_t> se(vals);
    for (int v = -128; v <= 127; ++v) {
        EXPECT_EQ(sb.In(VT(int8_t(v))), se.In(VT(int8_t(v))))
            << "Mismatch for " << v;
    }
}

TEST(CrossValidationTest, Int16SimdVsSet) {
    std::vector<int16_t> vals = {-100, 0, 100, 32767, -32768, 256};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<int16_t> sb(vals);
    SetElement<int16_t> se(vals);
    for (int v = -500; v <= 500; ++v) {
        EXPECT_EQ(sb.In(VT(int16_t(v))), se.In(VT(int16_t(v))))
            << "Mismatch for " << v;
    }
}
...
TEST(CrossValidationTest, Int32SimdVsSet) {
    ...
    std::vector<int32_t> probes = {-101,
                                   ...
                                   1000000};
    for (auto v : probes) {
        EXPECT_EQ(sb.In(VT(v)), se.In(VT(v))) << "Mismatch for " << v;
    }
}

TEST(CrossValidationTest, Int64SimdVsSet) {
    std::vector<int64_t> vals = {0, -1, 1, 1LL << 40, -(1LL << 40)};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<int64_t> sb(vals);
...
TEST(CrossValidationTest, FloatSimdVsSet) {
    std::vector<float> vals = {0.0f, -0.0f, 1.5f, -1.5f, 1e10f};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<float> sb(vals);
...
TEST(CrossValidationTest, DoubleSimdVsSet) {
    std::vector<double> vals = {0.0, -0.0, 1.5, -1.5, 1e100, 1e-100};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<double> sb(vals);
...
TEST(CrossValidationTest, StringFixedHashVsSet) {
    ...
    std::vector<std::string> probes = {"foo",
                                       ...
                                       std::string(501, 'x')};
    for (auto& v : probes) {
        EXPECT_EQ(fh.In(VT(v)), se.In(VT(v)))
...
TEST(SortVectorElementTest, Basic) {
    ...
    EXPECT_EQ(sv.Size(), 5);
    for (int v : {1, 2, 3, 4, 5}) {
        EXPECT_TRUE(sv.In(VT(int32_t(v))));
    }
    EXPECT_FALSE(sv.In(VT(int32_t(0))));
    EXPECT_FALSE(sv.In(VT(int32_t(6))));
}

TEST(SortVectorElementTest, StringWithStringView) {
    SortVectorElement<std::string> sv(
        std::vector<std::string>{"cherry", "apple", "banana"});
    EXPECT_TRUE(sv.In(VT(std::string("apple"))));
    EXPECT_TRUE(sv.In(VT(std::string_view("banana"))));
    EXPECT_FALSE(sv.In(VT(std::string("grape"))));
}
...
TEST(FlatVectorElementTest, Basic) {
    ...
    EXPECT_TRUE(fv.In(VT(int64_t(10))));
    EXPECT_TRUE(fv.In(VT(int64_t(30))));
    EXPECT_FALSE(fv.In(VT(int64_t(15))));
}
...
// SetElement<bool> uses a dedicated specialization (contains_true_/contains_false_)
// instead of ankerl hash set, to avoid ASan issues with set<bool>.
// Bool IN expressions are typically rewritten at the proxy layer, but this
// specialization serves as fallback.

TEST(SetElementBoolTest, GenericHashSet) {
    SetElement<bool> se(std::vector<bool>{true, false});
    EXPECT_TRUE(se.In(VT(true)));
    EXPECT_TRUE(se.In(VT(false)));
    EXPECT_EQ(se.Size(), 2);
}
...
// SetElement<bool> now uses the generic hash set (no specialization).
// GetElements() returns whatever ankerl hash set iterates.
...
TEST(FilterChunkTest, DeduplicatedFilterChunk) {
    ...
    std::vector<int32_t> vals_uniq = {7, 42};
    std::vector<int32_t> data(128);
    std::iota(data.begin(), data.end(), 0);
    VerifyFilterChunk(vals_dup, data, "DeduplicatedFilterChunk");
}
...
TEST(SetElementStringTest, AdversarialPrefixLarge) {
    ...
    for (int i = 1000; i < 1100; ++i) {
        std::string s = prefix + std::to_string(10000 + i);
        EXPECT_FALSE(fh.In(VT(s))) << "should not find: " << s;
    }
}
...
TEST(SetElementStringTest, AdversarialSameHashSameLength) {
    ...
    SetElement<std::string> se(vals);
    for (auto& v : vals) {
        EXPECT_TRUE(fh.In(VT(v))) << "FixedHash should find: " << v;
        EXPECT_TRUE(se.In(VT(v))) << "SetElement should find: " << v;
    }
    ...
    for (int i = kCount; i < kCount + 100; ++i) {
        ...
        EXPECT_EQ(fh.In(VT(s)), se.In(VT(s))) << "Disagreement for: " << s;
    }
}
...
TEST(SimdBatchElementTest, Int8Boundaries) {
    std::vector<int8_t> vals = {INT8_MIN, INT8_MAX, 0, -1, 1};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<int8_t> sb(vals);
    for (auto v : vals) {
        EXPECT_TRUE(sb.In(VT(v))) << "Missing: " << (int)v;
    }
    EXPECT_FALSE(sb.In(VT(int8_t(2))));
}

TEST(SimdBatchElementTest, Int16Boundaries) {
    std::vector<int16_t> vals = {INT16_MIN, INT16_MAX, 0, -1, 1};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<int16_t> sb(vals);
    for (auto v : vals) {
        EXPECT_TRUE(sb.In(VT(v))) << "Missing: " << v;
    }
    EXPECT_FALSE(sb.In(VT(int16_t(2))));
}

TEST(SimdBatchElementTest, Int32Boundaries) {
    std::vector<int32_t> vals = {INT32_MIN, INT32_MAX, 0, -1, 1};
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<int32_t> sb(vals);
    for (auto v : vals) {
        EXPECT_TRUE(sb.In(VT(v))) << "Missing: " << v;
...
TEST(SimdBatchElementTest, FloatSpecialValues) {
    // -0.0 and 0.0 compare equal in IEEE 754, so after dedup only one remains
    ...
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<float> sb(vals);
    EXPECT_TRUE(sb.In(VT(0.0f)));
    EXPECT_TRUE(sb.In(VT(-0.0f)));  // -0.0 == 0.0
    EXPECT_TRUE(sb.In(VT(std::numeric_limits<float>::max())));
    EXPECT_TRUE(sb.In(VT(std::numeric_limits<float>::lowest())));
    EXPECT_TRUE(sb.In(VT(std::numeric_limits<float>::min())));
    EXPECT_TRUE(sb.In(VT(std::numeric_limits<float>::denorm_min())));
    EXPECT_FALSE(sb.In(VT(1.0f)));
}
...
TEST(SimdBatchElementTest, DoubleSpecialValues) {
    ...
    std::sort(vals.begin(), vals.end());
    SimdBatchElement<double> sb(vals);
    EXPECT_TRUE(sb.In(VT(0.0)));
    EXPECT_TRUE(sb.In(VT(-0.0)));
    EXPECT_TRUE(sb.In(VT(std::numeric_limits<double>::max())));
    EXPECT_TRUE(sb.In(VT(std::numeric_limits<double>::lowest())));
    EXPECT_FALSE(sb.In(VT(1.0)));
}
...
TEST(SimdBatchElementTest, WrongTypeVariant) {
    SimdBatchElement<int32_t> sb(std::vector<int32_t>{1, 2, 3});
    EXPECT_TRUE(sb.In(VT(int32_t(1))));
    // Wrong type: std::get throws std::bad_variant_access
    EXPECT_THROW(sb.In(VT(int64_t(1))), std::bad_variant_access);
    EXPECT_THROW(sb.In(VT(int8_t(1))), std::bad_variant_access);
    EXPECT_THROW(sb.In(VT(float(1.0f))), std::bad_variant_access);
    EXPECT_THROW(sb.In(VT(double(1.0))), std::bad_variant_access);
    EXPECT_THROW(sb.In(VT(std::string("1"))), std::bad_variant_access);
    EXPECT_THROW(sb.In(VT(bool(true))), std::bad_variant_access);
    EXPECT_THROW(sb.In(VT(std::monostate{})), std::bad_variant_access);
}

TEST(SimdBatchElementTest, WrongTypeVariantFloat) {
    SimdBatchElement<float> sb(std::vector<float>{1.0f});
    EXPECT_TRUE(sb.In(VT(1.0f)));
    EXPECT_THROW(sb.In(VT(1.0)), std::bad_variant_access);  // double
    EXPECT_THROW(sb.In(VT(int32_t(1))), std::bad_variant_access);
}

TEST(SetElementTest, WrongTypeVariant) {
    SetElement<int64_t> se(std::vector<int64_t>{42});
    EXPECT_TRUE(se.In(VT(int64_t(42))));
    EXPECT_FALSE(se.In(VT(int32_t(42))));
    EXPECT_FALSE(se.In(VT(float(42.0f))));
    EXPECT_FALSE(se.In(VT(std::monostate{})));
}

TEST(SortVectorElementTest, WrongTypeVariant) {
    SortVectorElement<int32_t> sv(std::vector<int32_t>{10});
    EXPECT_TRUE(sv.In(VT(int32_t(10))));
    EXPECT_FALSE(sv.In(VT(int64_t(10))));
    EXPECT_FALSE(sv.In(VT(std::monostate{})));
}

TEST(FlatVectorElementTest, WrongTypeVariant) {
    FlatVectorElement<int64_t> fv(std::vector<int64_t>{5});
    EXPECT_TRUE(fv.In(VT(int64_t(5))));
    EXPECT_FALSE(fv.In(VT(int32_t(5))));
    EXPECT_FALSE(fv.In(VT(std::monostate{})));
}
...
TEST(FlatVectorElementTest, Uint8ForBoolIndex) {
    ...
    EXPECT_TRUE(fv.In(VT(uint8_t(1))));
    EXPECT_TRUE(fv.In(VT(uint8_t(0))));
    EXPECT_FALSE(fv.In(VT(uint8_t(2))));
...
TEST(SetElementStringTest, WrongTypeVariant) {
    ...
    EXPECT_TRUE(fh.In(VT(std::string("hello"))));
    EXPECT_TRUE(fh.In(VT(std::string_view("hello"))));
    EXPECT_FALSE(fh.In(VT(int32_t(0))));
    EXPECT_FALSE(fh.In(VT(bool(true))));
    EXPECT_FALSE(fh.In(VT(std::monostate{})));
}
...
TEST(SetElementTest, StringWithStringView) {
    ...
    EXPECT_TRUE(se.In(VT(std::string("alpha"))));
    EXPECT_TRUE(se.In(VT(std::string_view("beta"))));
    EXPECT_FALSE(se.In(VT(std::string("delta"))));
    EXPECT_FALSE(se.In(VT(std::string_view("ALPHA"))));
}
...
TEST(SortVectorElementTest, LargeList) {
    ...
    EXPECT_FALSE(sv.In(VT(int32_t(0))));
    EXPECT_FALSE(sv.In(VT(int32_t(1001))));
}
...
TEST(SetElementStringTest, BoundaryStringLengths) {
    ...
    EXPECT_EQ(fh.Size(), 5);
    for (auto& v : vals) {
        EXPECT_TRUE(fh.In(VT(v))) << "Missing: '" << v << "'";
    }
    // Near-misses
    EXPECT_FALSE(fh.In(VT(std::string("b"))));
    EXPECT_FALSE(fh.In(VT(std::string("1234567x"))));   // differ at byte 7
    EXPECT_FALSE(fh.In(VT(std::string("12345679"))));   // differ at byte 8
    EXPECT_FALSE(fh.In(VT(std::string("12345678x"))));  // differ beyond 8
}

TEST(SetElementStringTest, StringViewLookup) {
    SetElement<std::string> fh(std::vector<std::string>{"hello", "world"});
    EXPECT_TRUE(fh.In(VT(std::string_view("hello"))));
    EXPECT_TRUE(fh.In(VT(std::string_view("world"))));
    EXPECT_FALSE(fh.In(VT(std::string_view("HELLO"))));
}
...
// Bool IN is rewritten at proxy layer, but GetElementValues<bool> still works
// with the generic SetElement<bool> (ankerl hash set) as fallback.
...
// For varchar sealed segments, TermExpr uses T=string_view but the underlying
// element stores std::string. GetElementValues<string_view> returns empty
// because SetElement<string_view> doesn't exist. The production fix is in
// TermExpr.cpp: call GetElementValues<string> instead. These tests verify that
// GetElementValues<string> works correctly on string-based elements (the path
// used by skip-index for varchar sealed segments).
TEST(GetElementValuesTest, StringFromSetElementForSkipIndex) {
    auto elem = std::make_shared<SetElement<std::string>>(
    ...
    EXPECT_EQ(result.size(), 3);
    std::sort(result.begin(), result.end());
    EXPECT_EQ(result, (std::vector<std::string>{"foo", "hello", "world"}));
}

TEST(GetElementValuesTest, StringFromFlatVectorForSkipIndex) {
    auto elem = std::make_shared<FlatVectorElement<std::string>>(
...
TEST(GetElementValuesTest, StringFromSortVectorForSkipIndex) {
    auto elem = std::make_shared<SortVectorElement<std::string>>(
...
// GetElementValues<string_view> on string-based elements returns empty
// (no SetElement<string_view> exists). This is expected — production code
// should use GetElementValues<string> for the string_view case.
TEST(GetElementValuesTest, StringViewReturnsEmptyForStringElements) {
    auto elem = std::make_shared<SetElement<std::string>>(
        std::vector<std::string>{"a", "b"});
    auto result = GetElementValues<std::string_view>(
        std::static_pointer_cast<MultiElement>(elem));
    EXPECT_TRUE(result.empty());
}
...
TEST(FlatVectorElementTest, StringSmallIN) {
    ...
    EXPECT_TRUE(fv.In(VT(std::string("a"))));
    EXPECT_TRUE(fv.In(VT(std::string("bb"))));
    EXPECT_TRUE(fv.In(VT(std::string("ccc"))));
    EXPECT_FALSE(fv.In(VT(std::string("d"))));
    EXPECT_FALSE(fv.In(VT(std::string(""))));
}
...
// based on IN list size relative to simdLaneCount<T>() * 4 threshold.
// ═══════════════════════════════════════════════════════════════════════════
...
VerifyDispatchChoice(const std::string& label) {
    const size_t threshold =
        static_cast<size_t>(milvus::exec::simdLaneCount<T>()) * 4;
