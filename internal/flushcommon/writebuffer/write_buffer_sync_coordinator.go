...
type writeBufferSyncEntry struct {
	...
	submitted       bool
	failed          bool
	// checkpointPin is the WAL position this entry holds the channel checkpoint
	// at while its task is outstanding: the task's start position when it
	// carries payload, or the channel checkpoint at enqueue time for a
	// metadata-only flush/drop. Replaying the WAL from it regenerates the task.
	// checkpointPinKey is the syncCheckpoint registration to release — on
	// SUCCESS only; every failure path keeps the pin so replay stays anchored.
	checkpointPin    *msgpb.MsgPosition
	checkpointPinKey string
...
type writeBufferSyncQueue struct {
	...
	// queue, not per entry: a retry replays the segment from its oldest task, so
	// the queue advances as a unit.
	intent flushIntent
}
...
func (wb *writeBufferBase) finishWriteBufferSync(ctx context.Context, entry *writeBufferSyncEntry, task *syncmgr.SyncTask, taskErr error) error {
	...
	if taskErr != nil {
		...
		if entry.checkpointPinKey != "" {
			wb.syncCheckpoint.RemoveUnique(entry.checkpointPinKey)
		}
		if task.IsFlush() {
...
func (wb *writeBufferBase) getWriteBufferSyncTask(ctx context.Context, segmentInfo *metacache.SegmentInfo) (syncmgr.Task, error) {
	...
	// Every enqueued task pins the channel checkpoint at a position WAL replay
	// regenerates it from. A task with payload pins its start position: replay
	// redelivers the data and any fence behind it.
	//
	// A metadata-only task (startPos == nil) needs a pin too, but for a SEALED
	// segment the seal already installed one that is both earlier and longer
	// lived (metacache pendingFlushCheckpoint, see sealActionLocked), so pinning
	// again here would only add a second key for the same obligation.
	//
	// The fallback below is not dead: a metadata-only task can also be built for
	// a segment that is NOT Sealed/Flushing and therefore carries no fence pin —
	// a Dropped segment (GetDroppedSegmentPolicy / DropPartitions), or the final
	// per-segment task built by submitDropSegment on the drop-channel path. It
	// pins the channel checkpoint as of NOW: recovery is inclusive at the
	// checkpoint timetick (the seek MsgID is a last-confirmed position and a pack
	// whose end timetick equals the checkpoint is kept), so the drop message that
	// produced this task is redelivered.
	//
	// Either way the pin lives under the same wb.mut hold that yields the buffer
	// and registers the entry, so there is no window where the task exists
	// unpinned.
	checkpointPin := startPos
	if checkpointPin == nil && wb.checkpoint != nil && pendingFlushFence(segmentInfo) == nil {
		checkpointPin = typeutil.Clone(wb.checkpoint)
	}
	var checkpointPinKey string
	if checkpointPin != nil {
		checkpointPinKey = wb.syncCheckpoint.AddUnique(segmentID, checkpointPin, "syncing task")
	}
	...
	wb.registerWriteBufferSyncLocked(&writeBufferSyncEntry{
		...
		checkpointPin:    checkpointPin,
		checkpointPinKey: checkpointPinKey,
	})
	return task, nil
}
