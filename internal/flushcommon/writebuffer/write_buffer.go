...
type checkpointCandidates struct {
	...
	// pinSeq distinguishes AddUnique keys; see AddUnique.
	pinSeq atomic.Int64
}
...
// AddUnique registers a candidate under a key no other candidate can share, and
// returns that key for RemoveUnique. Plain Add keys by segmentID-timestamp, so
// two pins that happen to land on the same timestamp — e.g. a metadata-only
// flush pinned at the channel checkpoint next to a growing-source pin of the
// same segment — would overwrite each other, and the first Remove would strip
// the survivor's pin. The sequence suffix cannot appear in a plain key (it adds
// a third dash-separated field), so the two schemes never collide either.
func (c *checkpointCandidates) AddUnique(segmentID int64, position *msgpb.MsgPosition, source string) string {
	key := fmt.Sprintf("%d-%d-%d", segmentID, position.GetTimestamp(), c.pinSeq.Inc())
	c.candidates.Insert(key, &checkpointCandidate{segmentID, position, source})
	return key
}

// RemoveUnique releases a pin registered by AddUnique.
func (c *checkpointCandidates) RemoveUnique(key string) {
	c.candidates.Remove(key)
}
...
func newWriteBufferBase(channel string, metacache metacache.MetaCache, syncMgr syncmgr.SyncManager, option *writeBufferOption) (*writeBufferBase, error) {
	...
	if growingSourceResolver == nil {
		...
		growingSourceResolver = func(segmentID int64, endPos *msgpb.MsgPosition) (syncmgr.GrowingFlushSource, syncmgr.GrowingSourceState) {
			return syncmgr.DefaultGrowingSourceRegistry().Resolve(channel, segmentID, endPos)
		}
	}
	wb := &writeBufferBase{
		channelName:             channel,
...
func (wb *writeBufferBase) GetCheckpoint() *msgpb.MsgPosition {
	...
	for _, progress := range wb.growingSourceProgress {
		if position := progress.checkpointPosition(); position != nil {
	...
	for _, segment := range wb.metaCache.GetSegmentsBy(metacache.WithSegmentState(pendingFlushStates...)) {
		if position := pendingFlushFence(segment); position != nil {
			candidates = append(candidates, &checkpointCandidate{
				segmentID: segment.SegmentID(),
				position:  position,
				source:    "pending flush fence",
			})
		}
	}
...
// Dropped is NOT one of them: a dropped segment owes no flush, so its fence pin
// must stop holding (the drop task carries its own entry-level pin instead).
...
func pendingFlushFence(segment *metacache.SegmentInfo) *msgpb.MsgPosition {
	...
	if !lo.Contains(pendingFlushStates, segment.State()) {
		return nil
	}
	...
	futures, timedOut, err := wb.buildSyncSubmissionsWithAdmission(ctx,
		func(admission syncmgr.SyncTaskAdmission) ([]syncTaskSubmission, bool, error) {
			...
			if admission != nil && len(tasks) > 0 {
				if task, ok := tasks[0].(*syncmgr.SyncTask); ok {
				...
				// A write-buffer entry retains the slot across retries; a growing-source
				// submission releases it from its completion callback.
				transferred = true
			}
...
func (wb *writeBufferBase) submitDropSegment(ctx context.Context, segmentID int64) ([]*conc.Future[struct{}], *writeBufferSyncEntry, error) {
	...
	futures, timedOut, err := wb.buildSyncSubmissionsWithAdmission(ctx,
		func(admission syncmgr.SyncTaskAdmission) ([]syncTaskSubmission, bool, error) {
			...
			// Only the write-buffer path has a queue entry to hand back; the growing
			// path's caller waits on the future instead.
			if writeBufferTask, ok := task.(*syncmgr.SyncTask); ok {
				...
				writeBufferEntry.admission = admission
			}
		...
		if err != nil {
			if errors.Is(err, merr.ErrSegmentNotFound) {
				if wb.hasGrowingSourceProgress(segmentID) {
					mlog.Fatal(ctx, "growing-source progress outlived its metacache segment; refusing to discard checkpointed data",
						mlog.FieldSegmentID(segmentID),
...
func (wb *writeBufferBase) getSegmentsToSync(ts typeutil.Timestamp, policies ...SyncPolicy) []int64 {
	...
	return lo.Filter(segments, func(segmentID int64, _ int) bool {
		...
		// interval driveGrowingSourceRetries honors.
		if !progress.intent.due(time.Now(), wb.retryInterval()) && progress.intent.owes {
			return false
		}
		syncable := wb.growingSourceProgressSyncable(segmentID, progress)
		if syncable && progress.intent.owes {
...
func (wb *writeBufferBase) getSyncTask(ctx context.Context, segmentID int64) (syncmgr.Task, error) {
	...
	return wb.getWriteBufferSyncTask(ctx, segmentInfo)
}
...
func (wb *writeBufferBase) syncDropSegment(ctx context.Context, cancel context.CancelFunc, segmentID int64) error {
	for {
		...
		if err == nil {
			wb.mut.RLock()
			progress, ok := wb.growingSourceProgress[segmentID]
			dropPending := ok && progress.owesDrop
			wb.mut.RUnlock()
			if dropPending {
				continue
			}
...
func (wb *writeBufferBase) abortDrop(cancel context.CancelFunc) {
	...
	wb.mut.Lock()
	// buffers and growingSourceProgress are deliberately NOT cleared. They are
	// two of the three candidate sources GetCheckpoint pins on (the third is
	// syncCheckpoint); with all three empty it falls back to wb.checkpoint — the
	// latest CONSUMED position — which is past data this abort just declared
