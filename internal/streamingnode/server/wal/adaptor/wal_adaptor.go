...
type walAdaptorImpl struct {
	*roWALAdaptorImpl
	...
	chunkingEnabled        atomic.Bool                 // monotonic after StreamingVersionChunking is observed.
...
// records only after the cluster-wide chunking capability is enabled.
//
...
func (w *walAdaptorImpl) appendWithOptionalChunking(ctx context.Context, msg message.MutableMessage) (message.MessageID, error) {
	...
	// Reading chunks is a WAL-format capability. Do not create the first chunk
	// until StreamingCoord has observed every registered StreamingNode at the
	// compatible binary version and published the persisted feature version.
	// Fail fast during rolling upgrade instead of waiting or sending the
	// oversized unsplit record into the backend's infinite recoverable-error
	// retry loop.
	if !w.chunkingEnabled.Load() {
		if err := resource.Resource().CheckStreamingVersion(ctx, types.StreamingVersionChunking); err != nil {
			return nil, err
		}
		w.chunkingEnabled.Store(true)
		// Refresh the budget after crossing the capability boundary.
		chunkPayloadSize = w.chunkPayloadSize()
		if chunkPayloadSize <= 0 || len(msg.IntoMessageProto().GetPayload()) <= chunkPayloadSize {
			return w.appendOneWithRetry(ctx, msg)
		}
...
// configured (RocksMQ) or the configuration is invalid.
//
