...
// A channel with no local growing segment cannot be the only copy of anything:
// no drain call at all.
func (suite *ServiceSuite) TestStopSkipsDrainWithoutGrowingSegments() {
	suite.TestWatchDmChannelsInt64()
	suite.shortenGracefulStop()
	...
	suite.NoError(suite.node.Stop())
	suite.Zero(calls, "no local growing segment means no drain and no budget spent")
	_, ok := suite.node.delegators.Get(suite.vchannel)
	suite.False(ok)
}
