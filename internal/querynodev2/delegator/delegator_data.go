...
func (sd *shardDelegator) LoadSegments(ctx context.Context, req *querypb.LoadSegmentsRequest) error {
	...
	// segments that are ALREADY searchable: download and register their BM25
	// stats BEFORE the worker swaps the new column in, so there is no window
	// where the column is searchable while its stats are absent from the
	// oracle (empty results when avgdl is still 0, silently skewed IDF
	// otherwise). Activation happens after the worker load succeeds (in
	// handleReopenPostLoad); a download failure aborts the reopen before the
	// swap, so the window cannot open at all — querycoord retries the task.
