...
// fence -> ManualFlush -> drain that UnsubDmChannel performs, for every channel
// this node still holds growing segments on.
//
...
func (node *QueryNode) drainGrowingSourceFlushBeforeDestroy(stopStart time.Time) {
	...
	node.delegators.Range(func(channel string, sd delegator.ShardDelegator) bool {
		// No local growing segment on this channel means nothing here can be the
		// only copy of anything: skip it and spend no budget.
